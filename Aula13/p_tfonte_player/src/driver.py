...
class Driver():
    def __init__(self):
        self.publisher_command = rospy.Publisher('p_tfonte/cmd_vel', Twist, queue_size=1)
    ...
    def goalReceivedCallback(self, msg):
        ...
        try:
            self.pose = self.tf_buffer.transform(msg, 'p_tfonte/odom', rospy.Duration(1))
            goal_active = True
    ...
    def driveStraight(self, minimum_speed=0.1, maximum_speed=0.5):
        ...
        goal_in_base_link = self.tf_buffer.transform(goal_copy, 'p_tfonte/base_footprint', rospy.Duration(1))
